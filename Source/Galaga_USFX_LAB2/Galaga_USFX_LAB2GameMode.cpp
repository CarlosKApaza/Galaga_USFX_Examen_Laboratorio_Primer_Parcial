...
void AGalaga_USFX_LAB2GameMode::BeginPlay()
{
	...
	if (World != nullptr)
	{
		...
		GetWorldTimerManager().SetTimer(FTHDejarCaerBombasAleatorios, this, &AGalaga_USFX_LAB2GameMode::EliminarEnemigosAleatoriamente, 5.0f, true, 5.0f);

		// Temporizador para crear enemigos aleatorios después de 10 segundo
		GetWorldTimerManager().SetTimer(FTHCrearEnemigosAleatorios, this, &AGalaga_USFX_LAB2GameMode::CrearEnemigosAleatoriamente, 6.0f, true, 6.0f);
...
void AGalaga_USFX_LAB2GameMode::StopGameActions()
{
	...
	GetWorldTimerManager().ClearTimer(FTHDejarCaerBombasAleatorios); // Detiene la eliminación de enemigos aleatorios después de 40 segundos
